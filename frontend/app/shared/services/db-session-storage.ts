...
function createSessionExpiryTask(db: Kysely<DB>) {
    const task = new Task('session expiry', () => {
        return db.deleteFrom('session').where('expires', '<', new Date()).executeTakeFirst().then((result) => {
...
export function createDatabaseSessionStorage({
    ...
    return createSessionStorage({
        cookie, async createData(data, expires) {
            ...
            await db.updateTable("session").set({expires: new Date(now.getTime() + maxAge)}).where('id', '=', id).execute()
